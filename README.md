...
- **Special Characters Toggle** — When off (default), headlines with accented characters (é, ü, ñ) are filtered out. When on, accented headlines are included and guessing the base letter (e.g., `e`) reveals accented variants (`é`, `è`, `ê`). Styled as a toggle switch in the Difficulty section.
- **Duplicate Prevention** — Headlines are tracked per session to avoid repeats.
- **HTML Entity Decoding** — `&amp;`, `&quot;`, `&#39;` etc. are properly decoded.
- **Length Filter** — Only headlines between 10–150 characters are used.
...
### Settings UI
- **Consistent Section Layout** — Each section (Feeds, Exclude Words, Never Mask Words) uses a header with icon-only action buttons (refresh, copy) in the top-right corner.
...
- **Proxy URL in Settings** — Configure the CORS proxy URL from the Game settings section. An info button (ⓘ) opens a modal with full setup instructions and a copyable worker script.
...
### Keyboard Usability
- Increased vertical spacing between keyboard rows to reduce mistouches on mobile.
...
You can deploy via the **Cloudflare dashboard** (no CLI needed) or using the Wrangler CLI. The app includes a built-in setup guide — open **Settings → Game**, click the **ⓘ** button next to "CORS Proxy URL" for step-by-step instructions and a copyable worker script.
...
Finally, open the app, go to **Settings → Game**, and paste your worker URL into the **CORS Proxy URL** field. The app stores this in localStorage — no source code changes needed.
